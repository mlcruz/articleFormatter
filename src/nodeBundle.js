...
export class AbbrevIso {
  ...
  makeAbbreviation(value, languages = undefined, patterns = undefined) {
    if (patterns === undefined) patterns = this.getPotentialPatterns(value);
    ...
    for (const word of articles) {
      value = value.replace(
        new RegExp("((^|" + boundariesRegex.source + "))" + word + "\\s", "gu"),
        "$1"
      );
    ...
    value = value.replace(
      new RegExp("((^|" + boundariesRegex.source + "))(l|L|d|D)'", "gu"),
      "$1"
    );
    ...
    for (const word of this.shortWords_) {
      if (word.trim().length != 0) {
        ...
        result = result.replace(
          new RegExp("(" + boundariesRegex.source + ")" + cWord + "\\s", "gu"),
          "$1"
        );
      }
    }
